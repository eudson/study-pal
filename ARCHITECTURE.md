...
### Decisions made in absence — pending architect review

*(Autonomous `/goal` session, architect away; decided by the driver with a read-only advisor review — verdict REVISE→conditions met. Flagged here for the architect to confirm or revise. None reopen §10 above or alter the §5 state list.)*
...
- **2026-07-16 — Variant B has its OWN dedicated endpoints (`api/routers/variant_b.py`), reusing the pure services** (`project_for_child`, grading, `derive_gap_report`) rather than loosening the Variant-A endpoints. Rationale: A's marks are *published* (child-visible trust data, frozen `marks_published_at`/`published_visibility`); dedicated B endpoints mean A's write paths are byte-for-byte unchanged, so B cannot corrupt published A data *by construction* (advisor's top-ranked risk). B endpoints hard-target `variant="B"` and guard `state == GENERATING_B`.
